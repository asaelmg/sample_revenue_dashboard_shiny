...
server <- function(input, output) { 
  ...
  output$revenuebyPrd <- renderPlot({
    ggplot(data = recommendation, 
      ...
      geom_bar(position = "dodge", stat = "identity") + ylab("Revenue (in USD)") + 
      xlab("Product") + theme(legend.position="bottom" 
                              ,plot.title = element_text(size=15, face="bold")) + 
      ggtitle("Revenue by Product and Region") + labs(fill = "Region")
  })
  output$revenuebyRegion <- renderPlot({
    ggplot(data = recommendation, 
      ...
      xlab("Account") + theme(legend.position="bottom" 
                              ,plot.title = element_text(size=15, face="bold")) + 
      ggtitle("Revenue by Account and Region") + labs(fill = "Region")
  })
